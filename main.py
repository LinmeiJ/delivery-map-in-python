...
while ans:
    ...
    if ans == 1:
        user_input = input("Please enter by time (Example hour formate: 09:00 / 13:15):")
        my_utils.display_packages_by_time(user_input)
    elif ans == 2:
        pkg_id = int(input("Please enter a package ID number: "))
        time = input("Please enter a time (Example hour formate: 09:00 / 13:15):")
        my_utils.display_packages_by_time_and_id(pkg_id, time)
    elif ans == 3:
        my_utils.display_all_trucks_traveled_miles()
    elif ans == 4:
        print("\n Goodbye!")
        exit()
    else:
